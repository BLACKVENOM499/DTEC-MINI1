...
case 'tiktokdl': {
    try {
        ...
        if (!q) {
            return await socket.sendMessage(sender, { text: '*🚫 ᴘʟᴇᴀꜱᴇ ᴘʀොᴠɪᴅᴇ ᴀ ᴛɪᴋᴛොᴋ ᴠɪᴅᴇො ʟɪɴᴋ.*' }, { quoted: botMention });
        }

        if (!q.includes("tiktok.com")) {
            return await socket.sendMessage(sender, { text: '*🚫 ɪɴᴠᴀʟɪᴅ ᴛɪᴋᴛොᴋ ʟɪɴᴋ.*' }, { quoted: botMention });
        }

        // Parse username from URL
        const usernameMatch = q.match(/@([^\/]+)/);
        const username = usernameMatch ? usernameMatch[1] : 'ᴜɴᴋɴොᴡɴ';
        ...
        if (!data.status || !data.results) {
            return await socket.sendMessage(sender, { text: '*🚩 ꜰᴀɪʟᴇᴅ ᴛො ꜰᴇᴛᴄʜ ᴛɪᴋᴛොᴋ ᴠɪᴅᴇො.*' }, { quoted: botMention });
        }
...
`.trim();
        ...
        try {
            await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } });
            await socket.sendMessage(sender, { text: '*🚩 ꜱොᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀොɴɢ. ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇර්.*' });
        } catch(e){}
    }
    break;
}
