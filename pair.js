...
case 'tiktokdl': {
    try {
        ...
        if (!q) {
            return await socket.sendMessage(sender, { text: '*🚫 ᴘʟᴇᴀꜱᴇ ᴘʀොᴠɪᴅᴇ ᴀ ᴛɪᴋᴛොᴋ ʟɪɴᴋ.*' }, { quoted: botMention });
        }

        await socket.sendMessage(sender, { react: { text: '🎵', key: msg.key } });

        // API Request - Try New API first
        let apiUrl = `https://api.siputzx.my.id/api/d/tiktok/v2?url=${encodeURIComponent(q)}`;
        let response = await axios.get(apiUrl).catch(() => null);

        // If New API fails, use the alternative one
        if (!response || !response.data || !response.data.status) {
            apiUrl = `https:///movanest.xyz/v2/tiktok?url=${encodeURIComponent(q)}`;
            response = await axios.get(apiUrl).catch(() => null);
        }

        if (!response || !response.data || !response.data.data) {
            return await socket.sendMessage(sender, { text: '*🚩 ᴅᴏᴡɴʟᴏᴀᴅ ꜰᴀɪʟᴇᴅ. ᴘʟᴇᴀꜱᴇ ᴄʜᴇᴄᴋ ᴛʜᴇ ʟɪɴᴋ.*' }, { quoted: botMention });
        }

        const resData = response.data.data;
        // Check for media in different API formats
        const videoUrl = resData.meta?.media?.find(v => v.type === "video")?.org || resData.nowm || resData.video;

        if (!videoUrl) {
            return await socket.sendMessage(sender, { text: '*🚩 ᴠɪᴅᴇො ɴොᴛ ꜰොᴜɴᴅ ᴏʀ ᴘʀɪᴠᴀᴛᴇ.*' }, { quoted: botMention });
        }

        const captionMessage = `
🎵 *${botName.toUpperCase()} ᴛɪᴋᴛොᴋ ᴅොᴡɴʟොᴀᴅᴇʀ* 🎵
━━━━━━━━━━━━━━━━━━━━━━━━

👤 *ᴜꜱᴇʀ*: ${resData.author?.nickname || 'ᴜɴᴋɴොᴡɴ'}
📖 *ᴛɪᴛʟᴇ*: ${resData.title || 'ɴො ᴛɪᴛʟᴇ'}
👍 *ʟɪᴋᴇꜱ*: ${resData.like || '0'}
💬 *ᴄොᴍᴍᴇɴᴛꜱ*: ${resData.comment || '0'}
...
`.trim();
        ...
        // Internal error message and auto-reaction removal on fail
        try { 
            await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } });
            await socket.sendMessage(sender, { text: '*🚩 ꜱᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ. ᴛʀʏ ᴀɢᴀɪɴ!*' }); 
        } catch(e){}
    }
    break;
}
