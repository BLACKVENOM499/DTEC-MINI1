...
case 'tiktokdl': {
    try {
        ...
        // 🔹 Meta AI Fake Quote
        const botMention = {
            key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_FAKE_ID_TT" },
            message: {
                contactMessage: {
                    displayName: botName,
                    vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${botName};;;;\nFN:${botName}\nORG:ᴍᴇᴛᴀ ᴘʟᴀᴛꜰᴏʀᴍꜱ\nEND:VCARD`
        ...
        if (!q) {
            await socket.sendMessage(sender, { 
                text: '*🚫 ᴘʟᴇᴀꜱᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴛɪᴋᴛොᴋ ᴠɪᴅᴇᴏ ʟɪɴᴋ.*'
            }, { quoted: botMention });
            return;
        }
        ...
        if (!q.includes("tiktok.com")) {
            await socket.sendMessage(sender, { 
                text: '*🚫 ɪɴᴠᴀʟɪᴅ ᴛɪᴋᴛොᴋ ʟɪɴᴋ.*'
            }, { quoted: botMention });
            return;
        }

        await socket.sendMessage(sender, { react: { text: '🎵', key: msg.key } });
        await socket.sendMessage(sender, { text: '*⏳ ᴅොᴡɴʟොᴀᴅɪɴɢ ᴛɪᴋᴛොᴋ ᴠɪᴅᴇො...*' }, { quoted: botMention });

        // 🆕 Using the new API URL
        const apiUrl = `https://api.siputzx.my.id/api/d/tiktok/v2?url=${encodeURIComponent(q)}`;
        const { data } = await axios.get(apiUrl);

        if (!data.status || !data.data) {
            await socket.sendMessage(sender, { 
                text: '*🚩 ꜰᴀɪʟᴇᴅ ᴛො ꜰᴇᴛᴄʜ ᴛɪᴋᴛොᴋ ᴠɪᴅᴇො.*'
            }, { quoted: botMention });
            return;
        }
...
`.trim();
        ...
        // 📤 Sending Video without Buttons as requested
        await socket.sendMessage(sender, {
            video: { url: videoUrl },
        ...
        try {
            await socket.sendMessage(sender, { text: '*❌ ɪɴᴛᴇʀɴᴀʟ ᴇʀʀොʀ. ᴘʟᴇᴀꜱᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇර්.*' });
        } catch(e){}
    }
    break;
}
